...
import { FinanceStockService } from '../../service/stock';
import { FinanceStockQueryDTO } from '../../dto/stock'; // Assume a DTO is defined
...
export class FinanceStockEntityController extends BaseController {
  ...
  @Post('/page2')
  async page2(@Body() query: FinanceStockQueryDTO) {
    try {
      const result = await this.financeStockService.list(query);
      return this.ok({
        list: result.list,
